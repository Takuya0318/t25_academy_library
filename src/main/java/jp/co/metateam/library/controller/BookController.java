    ...
    public String register(@ModelAttribute BookMstDto bookMstDto, BindingResult result, RedirectAttributes ra) {
        try {
            ...
            if (bookMstDto.getIsbn() != null && !bookMstDto.getIsbn().isEmpty()
                    && bookMstDto.getIsbn().length() != 13) {
                result.rejectValue("isbn", "error.isbn.length", "ISBNは13桁で入力してください");
                hasError = true;
            }

            if (bookMstDto.getIsbn() != null && !bookMstDto.getIsbn().isEmpty()
                    && !bookMstDto.getIsbn().matches("^[0-9]+$")) {
                result.rejectValue("isbn", "error.isbn.hankaku", "ISBNは半角で入力してください");
                hasError = true;
            }

            // ✅ 削除済みを除外した重複チェック（←この部分を差し替え）
            String existingIsbn = bookMstService.searchActiveIsbn(isbnHolder);
            if (existingIsbn != null && !existingIsbn.isEmpty()) {
